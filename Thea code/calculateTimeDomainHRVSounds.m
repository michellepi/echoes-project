function [timeDomainHRV1, timeDomainHRV2] = calculateTimeDomainHRVSounds(heartsounds,missingHS)
...
for file = 1:numFiles
    % load data
    s1 = cell2mat(heartsounds(file,1)); size_org1 = size_org1 + size(s1,1);
    s2 = cell2mat(heartsounds(file,2)); size_org2 = size_org2 + size(s2,1);
...
%% display values
disp("HR and HRV using start of S1 and S2")
